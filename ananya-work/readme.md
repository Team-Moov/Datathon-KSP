...
Scope is ML/AI only ΓÇö no frontend/backend/dashboard here. All data is
...
squiggles in VS Code/Pylance ΓÇö those imports use runtime `sys.path`
...
indexes. Source of truth ΓÇö includes tables added mid-build
...
order (districts ΓåÆ persons ΓåÆ incidents ΓåÆ offenses ΓåÆ roles ΓåÆ ...).

### `data/synthetic.py`
Generates the entire synthetic dataset: 31 real Karnataka districts (with
placeholder socio-economic figures ΓÇö real ones are literal NCRB/NDAP rate
...
gives the network real community structure ΓÇö without it, node2vec has
nothing to learn), and ~200 financial transactions with seeded
structuring/funnel-account patterns.

### `features/graph_utils.py`
**Shared** multiplex-graph builder ΓÇö co-offending edges, MO-similarity
...
from here rather than each having their own copy (they used to ΓÇö that was
...
Co-offending graph ΓåÆ Louvain communities + PageRank + betweenness
centrality. Writes to `person_graph_metric` (append-only, versioned).
PageRank and betweenness are computed differently on purpose ΓÇö betweenness
...
any date." **Not currently wired into the risk-score model** ΓÇö for a
...
transformer ΓÇö this sandbox couldn't reach huggingface.co; upgrade path is
...
to `mo_linkage_cluster`. Validated result: recall@3 79%ΓåÆ90% over a raw-
...
predictions match edges that actually appeared since ΓÇö no synthetic label
...
(genuinely fails on `RandomSurvivalForest` ΓÇö falls back to permutation
...
0.71-0.75 vs. 0.50 baseline; **centrality dominates** ΓÇö permutation
...
District socio-economic indicators ΓåÆ one composite score, via percentile-
ranking (default, matches the real NDAP-based India precedent) or PCA.
Writes to `district_composite_index` (versioned ΓÇö separate from the
...
raw incident counts, one statewide coefficient (deliberately not GWR ΓÇö
cut from scope for hackathon-timeline stability, noted in the docstring).
Never joins to `Person`/`Case_Person_Role` ΓÇö stays strictly district-level
...
of truth ΓÇö both the training script and anything re-evaluating a saved
...
`N` depends on how deep the calling file is ΓÇö `eval/fairness_audit.py`
...
a real Python package ΓÇö there's no `__init__.py`/`setup.py` anywhere, by
...
# 7. Models (train then infer, in this order ΓÇö lead_rec and risk_score
...
Re-running any `train_*.py` + `infer.py` pair adds new versioned rows ΓÇö
